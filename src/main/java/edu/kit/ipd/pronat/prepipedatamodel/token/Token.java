...
/**
 * This class represents the internal representation of a token. Each token
 * consists of a word, position, pos tag, chunk and an instruction number.
 *
 * @author Markus Kocybik
 * @author Sebastian Weigelt - let class extend MainHypothesisToken on
 *         2016-12-08
...
public class Token extends MainHypothesisToken implements Comparable<Token> {
	...
	public Token(String word, POSTag pos, ChunkIOB chunkIOB, Chunk chunk, int position, int instructionNumber, String ner, String lemma,
	...
	public Token(MainHypothesisToken feed, POSTag pos, ChunkIOB chunkIOB, Chunk chunk, int position, int instructionNumber, String ner,
			String lemma, String stem, int sentenceNumber) {
		super(feed.getWord(), feed.getPosition(), feed.getConfidence(), feed.getType(), feed.getStartTime(), feed.getEndTime(),
				feed.getAlternatives());
		this.pos = pos;
		this.chunkIOB = chunkIOB;
		this.chunk = chunk;
		this.instructionNumber = instructionNumber;
	...
	public Token(MainHypothesisToken feed, POSTag pos, ChunkIOB chunkIOB, Chunk chunk, int position, int instructionNumber, String lemma,
			String stem) {
		this(feed, pos, chunkIOB, chunk, position, instructionNumber, null, lemma, stem, 0);
	}

	public Token(MainHypothesisToken feed, POSTag pos, ChunkIOB chunkIOB, Chunk chunk, int position, int instructionNumber, String ner,
			String lemma, String stem) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof Token) {
			...
			return super.equals(obj) && getPosition() == other.getPosition() && getChunk().equals(other.getChunk())
					&& getChunkIOB().equals(other.getChunkIOB()) && getInstructionNumber() == other.getInstructionNumber()
					&& getPos().equals(other.getPos()) && Objects.equals(getWord(), other.getWord())
					&& Objects.equals(getNer(), other.getNer()) && Objects.equals(getLemma(), other.getLemma())
					&& Objects.equals(getStem(), other.getStem());
		}
		return false;
	}
